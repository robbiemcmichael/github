...
-- TODO: Do this one as well, but not now because we'll get loads of merge conflicts
instance FromJSON RepoWebhookEvent where
    parseJSON = withText "RepoWebhookEvent" $ \s -> case T.toCaseFold s of
        ...
        _                                -> fail $ "Unknown RepoWebhookEvent " <> T.unpack s
